...
async function run(): Promise<void> {
  ...
  const enrichment = {
    ...
    generatedBy: { generator: "test", version: "1" },
    concepts: [{
      id: "concept-1",
      sourceId: source.sourceId,
      kind: "topic" as const,
  ...
  equal(loaded.enrichment?.concepts[0].label, "Hello", "enrichment should round-trip");
  ...
  equal(listed[0].conceptCount, 1, "summary should count concepts");
  ...
  let danglingConceptThrew = false;
  try {
    createSavedExploration({
      source,
      savedAt: saved.savedAt,
      ...
      enrichment: {
        ...enrichment,
        concepts: [{ ...enrichment.concepts[0], segmentIds: ["missing-segment"] }],
      },
    });
  } catch {
    danglingConceptThrew = true;
  }
  assert(danglingConceptThrew, "enrichment cannot reference evidence outside the exploration");
